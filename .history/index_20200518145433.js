...
connection.connect((error) => {
    ...
    admin.firestore().collection('transactions').get().then((result) => {
        ...
        const doneData = 0;
        console.log('Total Data Retrieved: '+totalData);
        result.docs.forEach((doc) => {
            ...
            var sql = `INSERT INTO transactions (uid, product_name, purchased_code, request_id, type, promo_code, status, amount, created_at) VALUES("${uid}", "${product_name}", "${purchased_code}", "${request_id}", "${type}", "${promo_code}", "${status}", "${amount}", "${created_at}")`;
