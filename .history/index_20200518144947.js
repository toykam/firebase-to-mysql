...
connection.connect((error) => {
    ...
    admin.firestore().collection('transactions').get().then((result) => {
        ...
        result.docs.forEach((doc) => {
            ...
            var sql = `INSERT INTO transactions (uid, product_name, purchased_code, request_id, type, promo_code, status, amount, created__at) VALUES("${uid}", "${message}", "${seen}", "${created_at}", "${seen_at}")`;
