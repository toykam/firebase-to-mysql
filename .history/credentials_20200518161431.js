...
connection.connect((error) => {
    ...
    admin.firestore().collection('credentials').get().then((result) => {
        ...
        result.docs.forEach((doc) => {
            ...
            var sql = `INSERT INTO credentials (commission, key_to_use, l_version, live_url, paystack_public_key, paystack_public_test_key, created_at)`;
            sql += ` VALUES("${commission}", "${key_to_use}", "${l_version}", "${live_url}", "${paystack_public_key}", "${paystack_public_test_key}", "${created_at}")`;
