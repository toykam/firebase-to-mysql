...
connection.connect((error) => {
    ...
    admin.firestore().collection('notifications').get().then((result) => {
        ...
        result.docs.forEach((doc) => {
            ...
            var url = oneTrans.url;
            var amount = oneTrans.amount;
            var sql = `INSERT INTO notifications (uid, amount, url, seen, title, message, created_at, seen_at)`;
            sql += ` VALUES("${uid}", "${amount}", "${url}", "${seen}", "${title}", "${message}", "${created_at}", "${seen_at}")`;
