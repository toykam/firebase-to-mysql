...
connection.connect((error) => {
    ...
    admin.firestore().collection('users').get().then((result) => {
        ...
        result.docs.forEach((doc) => {
            ...
            var device_id = oneTrans.device_id == null ? oneTrans.referal_code : oneTrans.device_id;
            var created_at = Date.now();
            var updated_at = Date.now();

            var sql = `INSERT INTO users (uid, first_name, last_name, email, username, acct_type, phone_number, pin, referral_code, referred_by, device_id, created_at, updated_at)`;
            sql += ` VALUES("${uid}", "${first_name}", "${last_name}", "${email}", "${password}", "${acct_type}", "${phone_number}", "${pin}", "${referral_code}", "${referred_by}", "${device_id}", "${created_at}", "${updated_at}")`;
